...
void parseConfigFile( const std::string& configFilename, mediaio::Transcoder& transcoder, mediacore::Profile& profile )
{
	...
	while( std::getline( configFile, line ) )
	{
		...
		if( std::getline( is_line, filename, '=' ) )
		{
			...
			if( std::getline( is_line, streamId, ':' ) )
			{
				...
				{
					// video
					mediacore::VideoFrameDesc imageDesc;
					imageDesc.setWidth( dummyWidth );
					imageDesc.setHeight( dummyHeight );
					imageDesc.setDar( dummyWidth, dummyHeight );
					mediacore::Pixel inputPixel( dummyPixelFormat );
					imageDesc.setPixel( inputPixel );

					mediaio::VideoCodec inputVideoCodec( mediaio::eCodecTypeEncoder, dummyVideoCodec );
...
int main( int argc, char** argv )
{
	if( argc != 3 )
	{
		std::cout << "mediaprocessor require an input config file and an output media filename" << std::endl;
		return( -1 );
	}
	...
	try
	{
		...
		mediacore::Profile profiles( true );

		if( verbose )
			std::cout << "output file: " << argv[2] << std::endl;

		std::string inputConfigFile( argv[1] );
		mediaio::OutputFile outputFile( argv[2] );

		mediaio::Transcoder transcoder( outputFile );
		...
		mediacore::ConsoleProgress progress;
