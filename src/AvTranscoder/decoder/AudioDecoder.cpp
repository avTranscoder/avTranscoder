...
bool AudioDecoder::decodeNextFrame(Frame& frameBuffer)
{
    ...
    size_t decodedSize =
        av_samples_get_buffer_size(NULL, avCodecContext.channels, _frame->nb_samples, avCodecContext.sample_fmt, 1);
    if(decodedSize == 0)
        return false;
...
bool AudioDecoder::decodeNextFrame(Frame& frameBuffer, const size_t subStreamIndex)
{
    ...
    size_t decodedSize =
        av_samples_get_buffer_size(NULL, output_nbChannels, _frame->nb_samples, avCodecContext.sample_fmt, output_align);
