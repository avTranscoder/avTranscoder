...
bool AudioTransform::init( const mediacore::Frame& srcFrame, const mediacore::Frame& dstFrame )
{
	const mediacore::AudioFrame& src = static_cast<const mediacore::AudioFrame&>( srcFrame );
	const mediacore::AudioFrame& dst = static_cast<const mediacore::AudioFrame&>( dstFrame );
...
void AudioTransform::convert( const mediacore::Frame& srcFrame, mediacore::Frame& dstFrame )
{
	if( ! _isInit )
	...
	int nbOutputSamples = nbOutputSamplesPerChannel * static_cast<const mediacore::AudioFrame&>( dstFrame ).desc().getChannels();
	
	if( dstFrame.getSize() != nbOutputSamples )
		dstFrame.getBuffer().resize( nbOutputSamples, 0 );
	static_cast<mediacore::AudioFrame&>( dstFrame ).setNbSamples( static_cast<const mediacore::AudioFrame&>( srcFrame ).getNbSamples() );
}
