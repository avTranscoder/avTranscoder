...
StreamTranscoder::StreamTranscoder(
		IInputStream& inputStream,
		OutputFile& outputFile,
		const mediacore::Profile::ProfileDesc& profile,
	...
	, _infinityStream( false )
{
	...
	switch( _inputStream->getStreamType() )
	{
		case AVMEDIA_TYPE_VIDEO :
		{
			...
			mediacore::VideoFrameDesc outputFrameDesc = _inputStream->getVideoCodec().getVideoFrameDesc();
			outputFrameDesc.setParameters( profile );
			outputVideo->setProfile( profile, outputFrameDesc );
			
			_outputStream = &outputFile.addVideoStream( outputVideo->getVideoCodec() );

			_sourceBuffer = new mediacore::VideoFrame( _inputStream->getVideoCodec().getVideoFrameDesc() );
			_frameBuffer = new mediacore::VideoFrame( outputVideo->getVideoCodec().getVideoFrameDesc() );
			
			_transform = new mediatransform::VideoTransform();
		...
		case AVMEDIA_TYPE_AUDIO :
		{
			...
			mediacore::AudioFrameDesc outputFrameDesc( _inputStream->getAudioCodec().getFrameDesc() );
			outputFrameDesc.setParameters( profile );
			if( subStreamIndex > -1 )
			{
				// @todo manage downmix ?
			...
			mediacore::AudioFrameDesc inputFrameDesc( _inputStream->getAudioCodec().getFrameDesc() );
			if( subStreamIndex > -1 )
				inputFrameDesc.setChannels( 1 );
			
			_sourceBuffer = new mediacore::AudioFrame( inputFrameDesc );
			_frameBuffer  = new mediacore::AudioFrame( outputAudio->getAudioCodec().getFrameDesc() );
			
			_transform = new mediatransform::AudioTransform();
	...
	, _infinityStream( false )
{
	...
	if( profile.find( mediaProfileType )->second == mediaProfileTypeVideo )
	{
		...
		mediacore::VideoFrameDesc inputFrameDesc = inputVideoCodec.getVideoFrameDesc();
		mediacore::VideoFrameDesc outputFrameDesc = inputFrameDesc;
		outputFrameDesc.setParameters( profile );
		_sourceBuffer = new mediacore::VideoFrame( inputFrameDesc );
		_frameBuffer  = new mediacore::VideoFrame( outputFrameDesc );
	...
	else if( profile.find( mediaProfileType )->second == mediaProfileTypeAudio )
	{
		...
		// Create inputFrame, and outputFrame which is based on a given profile
		mediacore::AudioFrameDesc inputFrameDesc = inputAudioCodec.getFrameDesc();
		mediacore::AudioFrameDesc outputFrameDesc = inputFrameDesc;
		outputFrameDesc.setParameters( profile );
		_sourceBuffer = new mediacore::AudioFrame( inputFrameDesc );
		_frameBuffer  = new mediacore::AudioFrame( outputFrameDesc );
