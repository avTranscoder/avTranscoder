...
bool OutputFile::wrap( const mediacore::CodedData& data, const size_t streamId )
{
	if( ! data.getSize() )
		return true;
	if( _verbose )
...
void OutputFile::setProfile( const mediacore::Profile::ProfileDesc& desc )
{
	using namespace mediacore::constants;
	if( ! desc.count( mediaProfileFormat ) )
	{
		throw std::runtime_error( "The profile " + desc.find( mediaProfileIdentificatorHuman )->second + " is invalid." );
	}
	
	if( ! mediacore::matchFormat( desc.find( mediaProfileFormat )->second, _filename ) )
	{
		throw std::runtime_error( "Invalid format according to the file extension." );
	}
	_outputFormat = av_guess_format( desc.find( mediaProfileFormat )->second.c_str(), _filename.c_str(), NULL);
